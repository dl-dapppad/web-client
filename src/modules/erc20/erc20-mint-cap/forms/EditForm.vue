...
const token = useProductErc20MintBurnCap(route.params.contractAddress as string)
...
const init = async () => {
  ...
  if (provider.value.selectedAddress) {
    const [balanceAmount, capAmount] = await Promise.all([
  ...
  overviewRows.value[2].value = formatAmount(
    cap.value,
    token.decimals.value,
    token.symbol.value,
  )
  overviewRows.value[3].value = String(token.decimals.value)
  overviewRows.value[4].value = token.owner.value
  overviewRows.value[5].value = formatAmount(
    balance.value,
    token.decimals.value,
    token.symbol.value,
  )
...
const headingData = {
  ...
  subtitle: t('product.erc20.mint-cap'),
...
<template>
  <base-edit-form :heading-data="headingData" :overview="formOverviewData">
    ...
    <template #tabSecond>
      <mint-form :token="token" @change-balance="updateBalances" />
      <approve-form :token="token" />
      <transfer-form
        :token="token"
        :balance="formBalance"
        @change-balance="updateBalances"
      />
      <transfer-from-form :token="token" @change-balance="updateBalances" />

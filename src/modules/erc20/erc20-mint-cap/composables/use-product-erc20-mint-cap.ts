...
import {
  ERC20MintCapp,
  ERC20MintCapp__factory,
} from '@/modules/erc20/erc20-mint-cap/types'

export interface ProductErc20MintCapContract
...
export const useProductErc20MintCap = (
  contractAddress?: string,
): ProductErc20MintCapContract => {
  const { provider } = storeToRefs(useWeb3ProvidersStore())

  const _instance = ref<ERC20MintCapp | undefined>()
  const _instance_rw = ref<ERC20MintCapp | undefined>()
  ...
  const init = (contractAddress: string): void => {
    ...
    if (provider.value.currentProvider) {
      _instance.value = ERC20MintCapp__factory.connect(
    ...
    if (provider.value.currentSigner) {
      _instance_rw.value = ERC20MintCapp__factory.connect(
        erc20Common.value.address,
  ...
  const encodeFunctionData = (
    initializeDataValues: unknown[],
    functionMethod = 'ERC20MintCapp_init',
  ): string => {
    const abiInterface =
      ERC20MintCapp__factory.createInterface() as utils.Interface

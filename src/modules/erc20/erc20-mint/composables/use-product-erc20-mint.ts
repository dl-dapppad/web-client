...
import { ERC20Mint, ERC20Mint__factory } from '@/modules/erc20/erc20-mint/types'

export interface ProductErc20MintContract extends ProductErc20CommonContract {
  encodeFunctionData: (
...
export const useProductErc20Mint = (
  contractAddress?: string,
): ProductErc20MintContract => {
  const { provider } = storeToRefs(useWeb3ProvidersStore())

  const _instance = ref<ERC20Mint | undefined>()
  const _instance_rw = ref<ERC20Mint | undefined>()
  ...
  const init = (contractAddress: string): void => {
    ...
    if (provider.value.currentProvider) {
      _instance.value = ERC20Mint__factory.connect(
    ...
    if (provider.value.currentSigner) {
      _instance_rw.value = ERC20Mint__factory.connect(
        erc20Common.value.address,
  ...
  const encodeFunctionData = (
    initializeDataValues: unknown[],
    functionMethod = 'ERC20Mint_init',
  ): string => {
    const abiInterface = ERC20Mint__factory.createInterface() as utils.Interface
